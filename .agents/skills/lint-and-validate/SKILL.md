...
#### Node.js / TypeScript / Astro (Bun)
1. **Lint/Fix:** `bun run lint` or `bun x eslint "path" --fix`
2. **Types:** `bun x tsc --noEmit`
3. **Astro check:** `bun run check`
...
## The Quality Loop
1. **Write/Edit Code**
2. **Run Audit:** `bun run lint && bun x tsc --noEmit && bun run check`
